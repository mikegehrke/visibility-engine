...
export default function DashboardTopBar({ 
  ...
  return (
    <header className="h-14 min-h-[56px] border-b border-border bg-smoke/95 backdrop-blur-sm px-4 lg:px-6 flex items-center justify-between">
      {/* Left: Toggle + Menu Button (mobile) + Logo (mobile) */}
      <div className="flex items-center gap-2">
        {/* Desktop Sidebar Toggle - nur wenn Sidebar eingeklappt */}
        {sidebarCollapsed && (
          <button
            onClick={onToggleSidebar}
            className="hidden lg:flex p-2 -ml-2 rounded-lg text-ink hover:bg-surface-1 active:bg-surface-2 transition-colors"
        ...
        <button
          onClick={onMenuClick}
          className="p-2 -ml-2 rounded-lg text-ink hover:bg-surface-1 active:bg-surface-2 transition-colors lg:hidden touch-target"
        ...
        {/* Dashboard Logo - Mobile only ODER wenn Sidebar collapsed */}
        <a 
          href="/dashboard" 
          className={`flex items-center gap-2.5 group ${sidebarCollapsed ? '' : 'lg:hidden'}`}
        >
