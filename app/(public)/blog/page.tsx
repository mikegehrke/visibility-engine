...
export default function BlogPage() {
  ...
  const allPosts = [
    { ...t.blog.posts.post1, key: 'post1', category: 'strategy' as CategoryKey, featured: true },
    { ...t.blog.posts.post2, key: 'post2', category: 'aiTechnology' as CategoryKey, featured: true },
    { ...t.blog.posts.post3, key: 'post3', category: 'strategy' as CategoryKey, featured: false },
    { ...t.blog.posts.post4, key: 'post4', category: 'aiTechnology' as CategoryKey, featured: false },
    { ...t.blog.posts.post5, key: 'post5', category: 'systems' as CategoryKey, featured: false },
    { ...t.blog.posts.post6, key: 'post6', category: 'analytics' as CategoryKey, featured: false },
  ];
    ...
    <div className="min-h-screen">
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative max-w-5xl mx-auto px-6 pt-32 pb-16">
          <div className="max-w-3xl">
            <Overline className="mb-6 block text-signal">{t.blog.hero.overline}</Overline>
            <H1 className="mb-6">
              {t.blog.hero.title}<br />
              <span className="text-signal">{t.blog.hero.titleHighlight}</span>
            </H1>
      ...
      {featuredPosts.length > 0 && (
        <section className="max-w-5xl mx-auto px-6 py-16">
          <div className="mb-8">
            <H2 className="text-xl">{t.blog.sections.featured}</H2>
          </div>
                ...
                <Card variant="elevated" padding="spacious" hover className="h-full">
                  <CardContent>
                    <div className="flex items-center gap-3 mb-4">
                      ...
                      <span className="text-xs text-slate">{post.readingTime} {t.blog.readingTime}</span>
                    </div>
      ...
      <section className={`border-t border-border bg-surface-1 ${featuredPosts.length === 0 ? 'pt-8' : ''}`}>
        <div className="max-w-5xl mx-auto px-6 py-16">
          <div className="mb-8">
            <H2 className="text-xl">{t.blog.sections.allPosts}</H2>
          </div>
                  ...
                  <CardContent className="flex flex-col md:flex-row md:items-center gap-4 md:gap-8 py-2">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <span className="text-xs text-slate hidden sm:inline">{post.readingTime} {t.blog.readingTime}</span>
                      </div>
      ...
      <section className="border-t border-border">
        <div className="max-w-3xl mx-auto px-6 py-24 text-center">
          <H2 className="mb-4">{t.blog.newsletter.title}</H2>
          ...
          {newsletterStatus === 'success' ? (
            <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg p-6 max-w-md mx-auto">
              ...
              <Small className="text-green-600 dark:text-green-400 mt-2 block">
                {language === 'de' ? 'Du erhältst bald Updates.' : "You'll receive updates soon."}
              </Small>
            </div>
          ) : (
          ...
          {newsletterStatus === 'error' && (
            <Small className="text-red-500 mt-4 block">
              {language === 'de' ? 'Fehler beim Abonnieren. Bitte versuche es erneut.' : 'Error subscribing. Please try again.'}
            </Small>
          )}
          {newsletterStatus !== 'success' && (
            <Small className="text-slate mt-4 block">
              {t.blog.newsletter.joinCount}
            </Small>
          )}
        </div>
      </section>
